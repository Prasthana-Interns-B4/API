class Device < ApplicationRecord
  ...
  before_create :status,:image_url,:employee_id_check
  before_update :status,:image_url,:employee_id_check
  before_create :tag_no

  validates :name,:tag_no,uniqueness: true
  attribute :category,:string,default: "Goodies"
  scope :search_bar, ->(search) {where("name ILIKE ? OR category ILIKE ? OR device_type ILIKE ?","%#{search}%","%#{search}%","%#{search}%").all}
  ...
  def tag_no
    self.tag_no = "DEV-" + "#{self.id.to_s.rjust(3,'0')}"
  end
